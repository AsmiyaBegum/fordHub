...
@HiltViewModel
class FordServiceViewModel  @Inject constructor(val navManager: NavManager ) :
    ViewModel() {
    ...
    fun getBookedVehicleServiceDetails() {
        networkScope.launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    _fordBookedServices.value = result
//                    _fordBookedServices.value = listOf(VehicleServiceDetail(1,"Asmiya benz",
//                        "Praveen Model A1","Wiper","10/09/2023","Chennai"))
    ...
    fun insertBookedVehicleServiceDetails(vehicleServiceDetail: VehicleServiceDetail) {
        networkScope.launch {
            try {
                fordVehicleRepository.insertServiceDetail(vehicleServiceDetail)
...
//                }
    ...
    fun serviceDetails(){
        networkScope.launch {
            try {
                val result = fordVehicleRepository.serviceDetails()

                withContext(Dispatchers.Main) {
                    result.onSuccess { serviceDetails ->
                        _serviceDetail.value = serviceDetails
                    }
                    result.onFailure {
                        _errorDetail.value = Unit
                    }
